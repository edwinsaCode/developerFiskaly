...
func TestBudgetCategory_HPPvsExpense_Split(t *testing.T) {
	capd := []budget.BudgetCategory{
		...
		budget.BudgetCategorySoft, budget.BudgetCategoryFinancing,
	}
	exp := []budget.BudgetCategory{budget.BudgetCategoryMarketing, budget.BudgetCategoryOther}
...
func TestGetBudgetedHPPPool(t *testing.T) {
	...
	add := func(cat budget.BudgetCategory, amt int64) {
		if _, err := svc.AddItem(ctx, 1, budget.AddItemRequest{
			PlanID: plan.ID, Category: cat, BudgetedAmount: rupiah(amt), Description: string(cat),
		}); err != nil {
	...
	add(budget.BudgetCategorySoft, 500_000_000)
	add(budget.BudgetCategoryFinancing, 300_000_000)
	...
	if pool.Land.String() != "2000000000" || pool.Hard.String() != "5000000000" ||
		pool.Soft.String() != "500000000" || pool.Financing.String() != "300000000" {
		t.Errorf("pool per kategori salah: %+v", pool)
	}
	// marketing (400jt) + other (100jt) TIDAK masuk pool HPP.
	if pool.Total().String() != "7800000000" {
		t.Errorf("total HPP pool harus 7.800.000.000 (tanpa marketing/other), got %s", pool.Total())
	}
}
